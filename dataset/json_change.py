...
def generate_random_date():
    """Generate a random date between 1900-01-01 and 2024-01-01."""
...
def generate_random_url():
    ...
    return f"https://random-site-{random.randint(1, 1000)}.com"


def modify_json_with_changes(original_json, num_changes):
    """
    Modify a limited number of date and URL fields in the JSON.

    Args:
        original_json (str): JSON as a string.
        num_changes (int): Number of fields to randomly modify.

    Returns:
        tuple: A list of changes and the modified JSON object.
    """
    changes = []
    modified_json = json.loads(original_json)  # Parse the JSON string
    fields_to_modify = []

    def collect_fields(obj, path=""):
        """Collect paths to all date and URL fields."""
        if isinstance(obj, dict):
            for key, value in obj.items():
                collect_fields(value, f"{path}.{key}" if path else key)
        elif isinstance(obj, list):
            for index, item in enumerate(obj):
                collect_fields(item, f"{path}[{index}]")
        else:
            if "date" in path.lower() and isinstance(obj, str):
                fields_to_modify.append((path, "date"))
            elif "url" in path.lower() and isinstance(obj, str):
                fields_to_modify.append((path, "url"))

    def set_field(obj, path, new_value):
        """Set a field in the JSON object based on its path."""
        keys = path.split(".")
        for i, key in enumerate(keys):
            if "[" in key and "]" in key:
                key, index = key[:-1].split("[")
                obj = obj[key][int(index)] if i < len(keys) - 1 else obj[key]
            elif i == len(keys) - 1:
                obj[key] = new_value
            else:
                obj = obj[key]

    # Collect all modifiable fields
    collect_fields(modified_json)

    # Randomly select fields to modify
    fields_to_change = random.sample(fields_to_modify, min(num_changes, len(fields_to_modify)))

    for field, field_type in fields_to_change:
        # Get the old value
        ...
        if field_type == "date":
            new_value = generate_random_date()
        elif field_type == "url":
            new_value = generate_random_url()
        else:
            continue
        ...
        # Update the JSON
        set_field(modified_json, field, new_value)

    return changes, modified_json
...
for _ in range(num_samples):
    ...
    changes, modified_json = modify_json_with_changes(original_json, num_changes)
    dataset.append({
        "changes": json.dumps(changes, indent=2),
        "original_json": json.dumps(json.loads(original_json), indent=2),
        "modified_json": json.dumps(modified_json, indent=2)
    })
